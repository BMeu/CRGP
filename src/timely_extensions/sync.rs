//! An extension to timely dataflow ``Scope``s allowing to wait for the computation to finish the
...
pub trait Sync<D1: Data, D2: Data> {
    ...
    ///
    /// Both ``input``s' times will be advanced. The computation ``self`` will step until the time
    /// of ``probe`` has reached the time of ``input1``.
