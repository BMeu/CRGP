...
mod vector_sorted {
    ...
    use super::sg::get_selected_friends;
    use super::sg::load;

    #[bench]
    fn iter(_bencher: &mut Bencher) {
        let mut sg: SocialGraph = SocialGraph::new();
        ...
        for (user, friends) in &sg {
            ...
            for friend in friends {
                ...
                stopwatch.lap();
                stopwatch.stop();
                if stopwatch.total_time() > 10_000 {
                    let time = stopwatch.total_time();
                    println!("  Friend {} in {}ns", fmt_thousands_sep(*friend as usize, ','), fmt_thousands_sep(time as usize, ','));
                }
            }
        }
    }
...
mod sg {
    ...
    pub type UserID = i64;
    pub type SocialGraph = HashMap<UserID, Vec<UserID>>;
    ...
    pub fn get_selected_friends(path: &PathBuf, out: &mut HashSet<UserID>) -> Result<()> {
